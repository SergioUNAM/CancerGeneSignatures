...
def _tone_color(tone: str) -> tuple[str, str]:
    """Return background and text colors for a tone name."""
    ...
    if tone == "danger":
        # vivid red for high-contrast accents (like the provided button)
...
def render_insight_board(
    *,
    ...
    def card_html(card: Card) -> str:
        ...
        abg, afg = _tone_color(card.accent_tone)
        chips = "".join(
            (
    ...
    if v == "light":
        ...
        chip_bg = "rgba(203, 213, 225, 0.55)"  # slate-300
        shadow = "0 18px 38px rgba(15, 23, 42, 0.12)"
        banner_border = "rgba(15, 23, 42, 0.10)"
    else:
    ...
    st.markdown(
        ...
        #{container_id} .chip {{
            display:inline-block; background: {chip_bg}; color:{text}; border:1px solid {border};
