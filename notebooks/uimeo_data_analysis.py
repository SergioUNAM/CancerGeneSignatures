...
if (df := cargar_datos_excel(ruta_archivo)) is None:
    raise SystemExit("❌ Error crítico: No se pudo cargar el archivo")

# -----------------------------------------------------------------------------------
# Función para extraer datos clave (nombres de pruebas, pozos y genes) de un DataFrame
# -----------------------------------------------------------------------------------
def extraer_informacion_cg(df: pd.DataFrame) -> tuple:
    """
    Extrae información estructurada del DataFrame cargado.
    ...
    """
    # Primero, revisamos cuántas filas y columnas tiene el DataFrame.
    # Es como asegurarnos de que un libro tenga al menos ciertas páginas y capítulos.
    rows, cols = df.shape
    if rows < 4 or cols < 2:
        # Si el libro es demasiado corto, avisamos que el formato es inesperado.
        alert_message("El archivo no tiene el formato esperado (mínimo 4 filas y 2 columnas).")
        return [], [], []

    # Extraemos los nombres de las pruebas desde la primera fila, ignorando celdas vacías.
    ...
    # Retornamos la información extraída para usarla más adelante.
    return test_names, pozos, genes

# -----------------------------------------------------------------------------------
# Función para procesar valores CT y clasificarlos como Controles o Muestras
# -----------------------------------------------------------------------------------
def procesar_ct(df: pd.DataFrame, genes: list, nombre_controles: str, nombre_muestras: str) -> tuple:
    ...
    Args:
        ...
        genes (list): Lista de genes extraídos.
    ...
    if len(df) <= 2:
        alert_message("El DataFrame no tiene suficientes filas para buscar 'CT' en la fila 3 (índice 2).")
        return pd.DataFrame(), pd.DataFrame()

    # Obtenemos la fila 3 (índice 2) donde se supone que aparece "CT".
    fila_ct = df.iloc[2]
    if "CT" not in fila_ct.values:
        # Si no encontramos "CT", avisamos y terminamos.
        alert_message("No se encontró ninguna columna con 'CT' en la fila 3 (índice 2).")
        return pd.DataFrame(), pd.DataFrame()
    ...
    # Buscamos en qué columnas aparece "CT"
    for idx in np.where(fila_ct == "CT")[0]:
        # Obtenemos el nombre de la prueba de la primera fila. Si no está, asignamos un nombre genérico.
    ...
    # Unimos todos los DataFrames de controles o muestras en uno solo. Si la lista está vacía, asignamos un DataFrame vacío.
...
def flujo_procesamiento(ruta_archivo: str) -> dict:
    ...
    Returns:
        dict: Diccionario con los datos clave para análisis futuros.
    """
    ...
    # 2. Extraemos la información clave: nombres de pruebas, pozos y genes.
    test_names, pozos, genes = extraer_informacion_cg(df)

    # 3. Mostramos información básica para que el usuario sepa qué datos tenemos.
    for titulo, datos, filtro in [
        ...
        # Imprimimos el título en pantalla.
        display(Markdown(titulo))
        # Aplicamos un filtro simple a la lista de datos (por ejemplo, quitando el primer elemento o vacíos).
    ...
    # Indicamos que el proceso se completó exitosamente.
